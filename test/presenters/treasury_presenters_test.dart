...
void main() {
  group('TreasuryDashboardPresenter', () {
    ...
    test('liquidAccounts returns only active liquid top-level accounts', () async {
      when(mockStorage.loadAccounts()).thenAnswer((_) async => [
            _account(id: 'a1', category: AccountCategory.ewallet, balance: 500),
            _account(id: 'a2', category: AccountCategory.creditCard, balance: 200),
            _account(id: 'a3', category: AccountCategory.savings, balance: 100, parentAccountId: 'a1'),
          ]);
    ...
    test('liabilityAccounts returns creditCard/creditLine/bnpl', () async {
      when(mockStorage.loadAccounts()).thenAnswer((_) async => [
            _account(id: 'a1', category: AccountCategory.creditCard, balance: 300),
            _account(id: 'a2', category: AccountCategory.bnpl, balance: 150),
            _account(id: 'a3', category: AccountCategory.ewallet, balance: 1000),
          ]);
      await presenter.load();
      expect(presenter.liabilityAccounts.map((a) => a.id), containsAll(['a1', 'a2']));
      expect(presenter.liabilityAccounts.any((a) => a.id == 'a3'), isFalse);
    });

    test('totalLiquidCash sums liquid account balances', () async {
      when(mockStorage.loadAccounts()).thenAnswer((_) async => [
            _account(id: 'a1', category: AccountCategory.ewallet, balance: 1000),
            _account(id: 'a2', category: AccountCategory.bank, balance: 2500),
            _account(id: 'a3', category: AccountCategory.creditCard, balance: 500),
          ]);
      await presenter.load();
    ...
    test('netWorth = assets - liabilities', () async {
      when(mockStorage.loadAccounts()).thenAnswer((_) async => [
            ...
            _account(id: 'a2', category: AccountCategory.creditCard, balance: 1000),
          ]);
      await presenter.load();
      expect(presenter.netWorth, 4000);
    });

    test('pendingReceivables sums unReceived receivables for current month', () async {
    ...
    test('deleteAccount throws StateError when sub-accounts exist', () async {
      ...
      await presenter.load();
      expect(() => presenter.deleteAccount('parent'), throwsA(isA<StateError>()));
    });
    ...
    test('addAccount persists to storage', () async {
      await presenter.load();
      final account = _account(id: 'new1', category: AccountCategory.bank, balance: 0);
  ...
  group('LedgerPresenter', () {
    ...
    setUp(() {
      ...
      when(mockStorage.loadAccounts()).thenAnswer((_) async => [
            _account(id: 'gcash', category: AccountCategory.ewallet, balance: 1000),
    ...
    test('addTransaction increases account balance for inflow', () async {
      ...
      await presenter.addTransaction(_txn(id: 't1', accountId: 'gcash', amount: 200, type: TransactionType.inflow));
    ...
    test('addTransaction decreases account balance for outflow', () async {
      await _waitForLoad(presenter);
      await presenter.addTransaction(_txn(id: 't1', accountId: 'gcash', amount: 300, type: TransactionType.outflow));
      final gcash = presenter.accounts.firstWhere((a) => a.id == 'gcash');
    ...
    test('deleteTransaction reverses balance delta', () async {
      await _waitForLoad(presenter);
      await presenter.addTransaction(_txn(id: 't1', accountId: 'gcash', amount: 400, type: TransactionType.outflow));
    ...
    test('addTransfer creates 2 records with shared transferGroupId', () async {
      ...
      final txns = presenter.groupedTransactions.values.expand((l) => l).toList();
    ...
    test('addTransaction awards +25 XP for first-ever transaction', () async {
      await _waitForLoad(presenter);
      await presenter.addTransaction(_txn(id: 't1', accountId: 'gcash', amount: 100, type: TransactionType.outflow));
      verify(mockStats.addXp(25)).called(1);
    });

    test('filteredMonthInflow sums inflow for selected month', () async {
      when(mockStorage.loadTransactions()).thenAnswer((_) async => [
            _txn(id: 't1', accountId: 'gcash', amount: 500, type: TransactionType.inflow, month: '2026-03'),
            _txn(id: 't2', accountId: 'gcash', amount: 200, type: TransactionType.outflow, month: '2026-03'),
          ]);
      await presenter.load();
    ...
    test('setAccount filters to single-account transactions', () async {
      when(mockStorage.loadTransactions()).thenAnswer((_) async => [
            _txn(id: 't1', accountId: 'gcash', amount: 100, type: TransactionType.outflow, month: '2026-03'),
            _txn(id: 't2', accountId: 'bpi', amount: 200, type: TransactionType.outflow, month: '2026-03'),
          ]);
      await presenter.load();
      presenter.setMonth('2026-03');
  ...
  group('BillsReceivablesPresenter', () {
    ...
    test('markBillPaid marks bill paid and creates outflow transaction', () async {
      when(mockStorage.loadBills()).thenAnswer((_) async => [
      ...
      final capturedBills = verify(mockStorage.saveBills(captureAny)).captured.last
          as List<Bill>;
    ...
    test('recurring bills auto-generated when navigating to new month', () async {
      when(mockStorage.loadBills()).thenAnswer((_) async => [
  ...
  group('BudgetPresenter', () {
    ...
    test('spentFor sums outflow transactions for category in month', () async {
      when(mockStorage.loadTransactions()).thenAnswer((_) async => [
            _txn(id: 't1', accountId: 'a1', amount: 200, type: TransactionType.outflow, month: '2026-03', categoryId: 'food'),
            _txn(id: 't2', accountId: 'a1', amount: 150, type: TransactionType.outflow, month: '2026-03', categoryId: 'food'),
            _txn(id: 't3', accountId: 'a1', amount: 100, type: TransactionType.inflow, month: '2026-03', categoryId: 'food'),
          ]);
      await presenter.load();
      presenter.setMonth('2026-03');
    ...
    test('setBudget creates new budget for category', () async {
      ...
      final captured = verify(mockStorage.saveBudgets(captureAny)).captured.last
          as List<Budget>;
      expect(captured.any((b) => b.categoryId == 'food' && b.allocatedAmount == 3000), isTrue);
    });
    ...
    test('isOverBudget returns true when spent > allocated', () async {
      ...
      when(mockStorage.loadTransactions()).thenAnswer((_) async => [
            _txn(id: 't1', accountId: 'a1', amount: 1200, type: TransactionType.outflow, month: '2026-03', categoryId: 'food'),
          ]);
      await presenter.load();
      presenter.setMonth('2026-03');
  ...
  group('TreasuryHistoryPresenter', () {
    ...
    test('closePreviousMonthIfNeeded is idempotent', () async {
      ...
      when(mockStorage.loadTransactions()).thenAnswer((_) async => [
            _txn(id: 't1', accountId: 'a1', amount: 100, type: TransactionType.outflow, month: lastMonth),
          ]);
      await presenter.load();
    ...
    test('closePreviousMonthIfNeeded skips when no data exists for prior month', () async {
      // No bills, transactions, or receivables for prior month → no summary created
      await presenter.load();
      verifyNever(mockStorage.saveMonthlySummaries(any));
    });

    test('closePreviousMonthIfNeeded creates summary for prior month with data', () async {
      final lastMonth = previousMonth(toMonthKey(DateTime.now()));
      when(mockStorage.loadTransactions()).thenAnswer((_) async => [
            _txn(id: 't1', accountId: 'a1', amount: 500, type: TransactionType.inflow, month: lastMonth),
          ]);
      await presenter.load();
    ...
    test('currentMonthSummary computes live data for current month', () async {
      ...
      when(mockStorage.loadAccounts()).thenAnswer((_) async => [
            _account(id: 'a1', category: AccountCategory.ewallet, balance: 2000),
          ]);
      when(mockStorage.loadTransactions()).thenAnswer((_) async => [
            _txn(id: 't1', accountId: 'a1', amount: 1000, type: TransactionType.inflow, month: currentMonth),
            _txn(id: 't2', accountId: 'a1', amount: 300, type: TransactionType.outflow, month: currentMonth),
          ]);
      await presenter.load();
