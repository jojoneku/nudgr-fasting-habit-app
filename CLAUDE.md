...
8. **One owner per slice of state, and readers subscribe to the owner.** A presenter that shows data another presenter owns must mirror it off a listener — never keep a private copy refreshed only by its own `load()`, and never have the owner push a reload into its readers. `TreasuryDashboardPresenter` owns nothing and shows everything, so it is the worked example: `_syncFromLedger` (accounts, transactions, categories), `_syncFromBudget` (budgets, groups), `_syncFromBills` (bills, receivables, set-asides). A copy that only `load()` refreshes goes stale the moment another page edits it, and the surfaces that never reload (the Hub cards) stay wrong indefinitely. Push is correct only when subscribing would invert the dependency — e.g. bills handing outstanding-reimbursement ids *down* to the ledger.
