...
- **STATUS: ✅ DONE (storage-level + decision-logic contracts) — PR (`test/data-persistence-regression`).** `data_persistence_regression_test.dart` locks the named contracts: (1) sign-out detach + re-login restores; (2) backup export→wipe→import round-trips AND `importUserData` never enqueues (restore can't fight the cloud); (3) empty-payload classification (the `quests:[]` clobber predicate); (4) tombstone recognition. Plus the per-phase unit tests already merged (predicates, backoff, prune). **Follow-up (the remaining piece):** the full end-to-end push/pull scenarios (multi-device empty-overwrite, tombstone pull-reconcile, wipe→pull) need an injectable **`SyncBackend` seam** so `SyncService` can run against an in-memory fake — a deliberate, reviewed refactor rather than a brittle PostgREST-builder fake. Recommended next.
