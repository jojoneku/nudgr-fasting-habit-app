...
- **2.2 Wire queue before reload.** Set the user namespace + `setSyncQueue` **before** `_reloadAll()` so writes during load/migration/rollover are queued. Re-order `_initSync`.
- **2.3 Atomic migration.** Two-phase: write scoped, verify, then remove bare (or keep a migration-complete flag and only delete bare keys after the scoped namespace is confirmed). Idempotent + crash-safe.
- **Tests:** poison entry doesn't block tail; writes during reload are queued; crash-mid-migration leaves no data gap.

### Phase 3 — Hybrid completeness & cross-device correctness
- **3.1 Sync `notification_preferences`** (add to a singleton domain, push+pull). Audit remaining local-only keys; document each as "intentionally local" or fold into sync.
