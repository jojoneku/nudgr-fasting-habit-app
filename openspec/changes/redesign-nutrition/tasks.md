...
- [x] 6.1 Restyle `food_photo_sheet.dart`: source (Take photo / Choose from gallery) → preview (photo + note + Retake/Analyze) → **estimate** (items + P/C/F + total + Log it/Edit) → commit. Presenter split `parsePhoto`→`resolvePhotoPreview` (resolves to a pending estimate, no logging) + shared `commitPendingChat` (attaches thumbnail); discard deletes the orphan thumbnail. Log-it shows a "Logged" undo toast. `photoAi` attribution + no auto-learn preserved. 10 photo tests green. [analyze clean; on-device smoke pending]
